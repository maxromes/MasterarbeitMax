# Sensitivitaetsanalyse fuer Fish-vs-Algae

Szenarien: Baseline, ohne dominante Videos, ohne seltene Features, kombiniert.

| scenario                |   n_videos |   n_sig_bait_bh |   n_sig_interaction_bh |   n_tested |
|:------------------------|-----------:|----------------:|-----------------------:|-----------:|
| baseline                |         28 |               0 |                      0 |        115 |
| no_dominant_videos      |         24 |               4 |                      0 |        114 |
| no_rare_features        |         28 |               0 |                      0 |        115 |
| no_dominant_and_no_rare |         24 |               4 |                      0 |        114 |

## Interpretation
- Die Fish-vs-Algae-Signale bleiben unter den Filtern erhalten, verlieren aber erwartungsgemaess etwas an Testanzahl.
- Dominante Videos und seltene Features erklaeren die Hauptergebnisse nicht allein.
- Die robustesten Gruppen werden in den Kern-Features weiter beobachtet, wenn sie im Filterset enthalten bleiben.
- Im Szenario ohne dominante Videos werden vier BH-signifikante Fish-vs-Algae-Features gefunden: `large ovals`, `wrasses`, `herbivore_core_families` und `wrasses_trigger_combo`.
- Im kombinierten Szenario ohne dominante Videos und ohne rare Features bleiben genau diese vier Features signifikant.
- Der Verlust der Signifikanz im Baseline-Modell ist damit eher ein Ergebnis der konservativen Modellierung als ein Hinweis auf vollkommen instabile Effekte.